# =========================================================
# ひとくち中国語｜Shorts（やさしい中国語の短編ストーリー聞き流し）
# 【完全創作】Geminiが学習者向けのやさしい中国語を作成。
...
def generate_story(avoid_summaries, max_retries=5):
    ...
    prompt = f"""You are a Chinese (Mandarin, simplified characters) teacher creating listening material for Japanese learners.
Write a very short, easy-to-listen original story in Chinese (Mandarin, simplified characters) (level: {LEVEL}).

Rules:
- Completely original. No real people, brands, or specific place names.
- Use simple, natural, everyday Chinese (Mandarin, simplified characters) suitable for learners.
...
{{
  ...
  "title_tgt": "short title in Chinese (Mandarin, simplified characters)",
  "sentences": [
    {{"tgt": "one sentence in Chinese (Mandarin, simplified characters).", "ja": "その日本語訳"}}
  ]
}}
sentences: 6-9 items. Each 'tgt' is one natural Chinese (Mandarin, simplified characters) sentence with its 'ja' translation.{avoid_text}
"""
...
def build_video(data):
    ...
    title = data.get("youtube_title", "ひとくち中国語")
    safe = title
    for ch in r'\/:*?"<>|':
...
def upload(youtube, path, title):
    description = (
        "やさしい中国語の短いストーリーで、聞き流し中国語リスニング。\n"
        "現地語と和訳つき。作業用・通勤用にどうぞ。\n\n#中国語 #中国語リスニング #中国語学習 #聞き流し #shorts #Shorts"
    )
    body = {
        "snippet": {
            "title": (title + " #shorts")[:100],
            "description": description[:5000],
            "tags": ["中国語", "中国語リスニング", "中国語学習", "聞き流し", "作業用BGM", "ピンイン"],
            "categoryId": "27",
            "defaultLanguage": "ja",
        },
