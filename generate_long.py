# =========================================================
# ひとくち中国語｜長尺（やさしい中国語の短編ストーリー聞き流し・複数話）
...
Write a short, easy-to-listen original story in Chinese (Mandarin, simplified characters) (level: {LEVEL}).
...
def build_video(stories):
    ...
    main_title = stories[0].get("title_ja", "ひとくち中国語")
    safe = main_title
    for ch in r'\/:*?"<>|':
    ...
    # エンディング
    end_txt = "谢谢观看"
    a = make_audio_tgt("谢谢观看", f"a_{idx}.mp3")
...
def upload(youtube, path, title, stories):
    ...
    description = (
        "やさしい中国語の短いストーリーを集めた、聞き流し中国語リスニング（現地語＋和訳つき）。\n"
        "作業用・睡眠用・通勤用にどうぞ。\n"
        f"Stories: {titles}\n\n#中国語 #中国語リスニング #中国語学習 #聞き流し #作業用BGM"
    )
    body = {
        "snippet": {
            "title": title[:100],
